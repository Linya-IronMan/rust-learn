...
- 是匿名函数
- 保存为变量、作为参数
- 可以在一个地方创建闭包，然后在另一个 上下文中调用闭包来完成运算
- 可以从其定义的作用域捕获值

闭包是一个匿名函数，可以不标注参数以及返回值的类型。但是在调用过一次闭包之后，编译器会根据这次传入的参数以及返回值将闭包的参数和返回值类型确定下来，下次调用的时候必须遵守本次生成的类型规范。




**demo：匿名函数可以选择不书写参数与返回值类型**
...
// 此处会报错，因为对匿名函数传递的参数类型与上次调用不一样 10.82.49.17:8090 
