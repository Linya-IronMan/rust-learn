...
pub async fn health_check_handler(app_state: web::Data<AppState>) -> HttpResponse {
    let health_check_handler = &app_state.health_check_response; 
    // lock 就是将visit_count锁定，并获取其所有权
    let mut visit_count = app_state.visit_count.lock().unwrap();
    let response = format!("{} {} times", health_check_handler, visit_count);
    *visit_count += 1;
